...
const ROUTING_API_BASE_ALT = (process.env.EXPO_PUBLIC_ROUTING_API_BASE_ALT ?? 'https://routing.openstreetmap.de/routed-car').replace(
  /\/+$/,
  '',
);
const VALHALLA_API_BASE = (process.env.EXPO_PUBLIC_VALHALLA_API_BASE ?? 'https://valhalla1.openstreetmap.de').replace(/\/+$/, '');
...
function parseCoordinateText(address: string): GeoPoint | null {
  const directMatch = address.match(/(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)/);
  if (!directMatch) {
    return null;
  }
  ...
  return isLikelyPhilippines(point) ? point : null;
}
...
export async function geocodeAddress(address: string): Promise<GeoPoint | null> {
  ...
  const parsedCoordinate = parseCoordinateText(address);
  if (parsedCoordinate) {
    geocodeCache.set(query, parsedCoordinate);
...
function decodePolyline(encoded: string, precision = 6): GeoPoint[] {
  if (!encoded) {
    return [];
  }
...
async function fetchOsrmRoute(
  ...
  from: GeoPoint,
  to: GeoPoint,
): Promise<{ distanceKm: number; coordinates: GeoPoint[] } | null> {
  const routeUrl = `${baseUrl}/route/v1/driving/${from.longitude},${from.latitude};${to.longitude},${to.latitude}`;
...
async function fetchValhallaRoute(from: GeoPoint, to: GeoPoint): Promise<{ distanceKm: number; coordinates: GeoPoint[] } | null> {
  const response = await fetch(`${VALHALLA_API_BASE}/route`, {
    method: 'POST',
    headers: {
      Accept: 'application/json',
  ...
  const shape = typeof legs[0]?.shape === 'string' ? legs[0].shape : '';
  const distanceKmRaw = Number(summary?.length ?? 0);

  if (!shape || !Number.isFinite(distanceKmRaw) || distanceKmRaw <= 0) {
    return null;
  }
  ...
  return {
    distanceKm: Number(distanceKmRaw.toFixed(2)),
    coordinates,
  };
}
...
export async function fetchDrivingRoute(from: GeoPoint, to: GeoPoint): Promise<{ distanceKm: number; coordinates: GeoPoint[] } | null> {
  ...
  try {
    const osrmBases = [ROUTING_API_BASE, ROUTING_API_BASE_ALT].filter(Boolean);
    for (const base of osrmBases) {
      try {
        const osrmRoute = await fetchOsrmRoute(base, from, to);
        if (osrmRoute) {
          routeCache.set(key, osrmRoute);
          return osrmRoute;
        }
      } catch {
        // Continue trying the next OSRM-compatible endpoint.
      }
    }
  } catch {
  ...
  try {
    const valhallaRoute = await fetchValhallaRoute(from, to);
    if (valhallaRoute) {
      routeCache.set(key, valhallaRoute);
      return valhallaRoute;
    }
  } catch {
    // Fall through and cache null.
  }
