...
export function ProductDetailScreen() {
  ...
  const applyTypedQuantity = () => {
    const parsed = Number(quantityText.trim());
    const safeMax = Math.max(1, maxStock);
    const normalized = Number.isFinite(parsed) ? Math.max(1, Math.min(safeMax, Math.floor(parsed))) : 1;
    setQuantity(normalized);
    setQuantityText(String(normalized));
  };

  const detailPalette = theme.isDark
    ? {
        productCard: '#11284A',
        variantCard: '#0E3A31',
        qtyCard: '#4A2B0E',
  ...
  return (
    ...
    >
      ...
      <View style={[styles.infoCard, { backgroundColor: detailPalette.productCard, borderColor: theme.colors.border }]}>
      ...
      {activeVariants.length ? (
        <View style={[styles.variantWrap, { backgroundColor: detailPalette.variantCard, borderColor: theme.colors.border }]}>
      ...
      <View style={[styles.qtyCard, { backgroundColor: detailPalette.qtyCard, borderColor: theme.colors.border }]}>
      ...
      <View style={[styles.reviewCard, { backgroundColor: detailPalette.reviewCard, borderColor: theme.colors.border }]}>
