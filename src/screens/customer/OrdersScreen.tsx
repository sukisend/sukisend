...
export function OrdersScreen() {
  ...
  return (
    ...
    >
          ...
          <View key={order.id} style={[styles.card, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
            ...
            <View style={styles.actions}>
              <Pressable
                ...
                onPress={async () => {
                  try {
                    const events = await fetchOrderTrackingEvents(order.id);
      ...
      <Modal visible={Boolean(trackingOrder)} transparent animationType="slide" onRequestClose={() => setTrackingOrder(null)}>
          ...
          <View style={[styles.modalCard, { backgroundColor: theme.colors.card }]}>
            ...
            {trackingCoordinates.length ? (
              <MapView
                style={styles.map}
                initialRegion={{
                  latitude: trackingCoordinates[0].latitude,
                  longitude: trackingCoordinates[0].longitude,
                  latitudeDelta: 0.12,
                  longitudeDelta: 0.12,
                }}
              >
                {trackingCoordinates.map((point, index) => (
                  <Marker key={`${point.latitude}-${point.longitude}-${index}`} coordinate={point} />
                ))}
                {trackingCoordinates.length > 1 ? <Polyline coordinates={trackingCoordinates} strokeWidth={3} strokeColor="#F97316" /> : null}
              </MapView>
            ) : (
            ...
            <Pressable style={[styles.primaryButton, { backgroundColor: theme.colors.primary }]} onPress={() => setTrackingOrder(null)}>
